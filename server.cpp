...
void handle_metadata_request(int server_sock, struct sockaddr_in &client_addr, socklen_t &client_len, char* buffer) {
    ...
    Metadata net_meta;
    net_meta.file_size = htonll(meta.file_size);
    net_meta.num_chunk = htonll(meta.num_chunk);
    net_meta.chunk_size = htonll(meta.chunk_size);
  
    char message[BUFFER_SIZE];
    snprintf(message, sizeof(message), "META:%s:", filename);
    size_t header_len = strlen(message);
    if (header_len + sizeof(net_meta) <= BUFFER_SIZE - sizeof(uint32_t)) {
     memcpy(message + header_len, &net_meta, sizeof(net_meta));
     size_t data_len = sizeof(net_meta);
     size_t total_len_before_crc = header_len + data_len;
  
     uint32_t crc = calculate_crc32(message, total_len_before_crc);
     memcpy(message + total_len_before_crc, &crc, sizeof(uint32_t));
     size_t total_len = total_len_before_crc + sizeof(uint32_t);
  
     handle_reply_to_client(server_sock, client_addr, client_len, message, total_len);
     printf("Debug in meta func: Header: %s, Metadata size: %zu bytes, CRC: %u\n", message, sizeof(net_meta), crc);
     return;
    } else {
...
void handle_chunk_request(int server_sock, struct sockaddr_in &client_addr, socklen_t &client_len, char* buffer) {
    ...
    if (header_len + actual_chunk_size <= BUFFER_SIZE - sizeof(uint32_t)) {
     memcpy(reply_message + header_len, chunk_data, actual_chunk_size);
     size_t total_len_before_crc = header_len + actual_chunk_size;
     uint32_t crc = calculate_crc32(reply_message, total_len_before_crc);
     memcpy(reply_message + total_len_before_crc, &crc, sizeof(uint32_t));
     size_t total_len = total_len_before_crc + sizeof(uint32_t);
     handle_reply_to_client(server_sock, client_addr, client_len, reply_message, total_len);
    } else {
     std::cerr << "[SERVER] Phản hồi chunk quá lớn.\n";
     handle_reply_to_client(server_sock, client_addr, client_len, INTERNAL_ERROR, strlen(INTERNAL_ERROR));
    }
    close(fd);
}
...
/** HANDLER FUNCTIONS **/
void handle_reply_to_client(int server_sock, sockaddr_in &client_addr, socklen_t &client_len, char* buffer, size_t buffer_len) {
...
void handle_reply_from_client(int server_sock, sockaddr_in &client_addr, socklen_t &client_len, char* buffer, size_t buffer_len) {
