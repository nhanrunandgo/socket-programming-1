   ...
   if (token != NULL) {
    ...
    if (token != NULL && strcmp(token, "META") == 0) {
     ...
     if (token != NULL && strcmp(token, filename.c_str()) == 0) {
      size_t metadata_start = token - buffer + strlen(token) + 1;
      if (metadata_start + sizeof(Metadata) + sizeof(uint32_t) <= recv_len) {
       Metadata net_meta;
       memcpy(&net_meta, buffer + metadata_start, sizeof(Metadata));
       ...
       memcpy(&received_crc, buffer + metadata_start + sizeof(Metadata), sizeof(uint32_t));

       uint32_t calculated_crc = calculate_crc32(buffer, metadata_start + sizeof(Metadata));
       ...
       if (calculated_crc == received_crc) {
        ...
        std::cerr << "Lỗi CRC metadata cho \"" << filename << "\". CRC nhận được: " << received_crc << ", CRC tính toán: " << calculated_crc << "\n";
        return false;
       }
      } else {
       std::cerr << "Dữ liệu metadata không đầy đủ cho \"" << filename << "\"\n";
       return false;
      }
     } else {
         ...
         if (token != NULL && strcmp(token, filename.c_str()) == 0) {
          ...
          if (token != NULL) {
           ...
           if (received_chunk_id == chunk_id) {
            size_t data_start = token - buffer + strlen(token) + 1;
            if (data_start + sizeof(uint32_t) <= recv_len) {
             size_t chunk_data_len = recv_len - data_start - sizeof(uint32_t);
             if (chunk_data_len > 0) {
              std::vector<char> received_data(buffer + data_start, buffer + data_start + chunk_data_len);
              uint32_t received_crc;
              memcpy(&received_crc, buffer + data_start + chunk_data_len, sizeof(uint32_t));
              uint32_t calculated_crc = calculate_crc32(received_data.data(), received_data.size());
              ...
              if (calculated_crc == received_crc) {
               ...
               std::cerr << "Lỗi CRC chunk " << chunk_id << " của \"" << filename << "\". CRC nhận được: " << received_crc << ", CRC tính toán: " << calculated_crc << "\n";
              }
             } else {
              std::cerr << "Kích thước dữ liệu chunk " << chunk_id << " của \"" << filename << "\" bằng 0 hoặc không hợp lệ.\n";
             }
            } else {
             std::cerr << "Dữ liệu chunk " << chunk_id << " của \"" << filename << "\" không đầy đủ (thiếu CRC).\n";
            }
           } else {
